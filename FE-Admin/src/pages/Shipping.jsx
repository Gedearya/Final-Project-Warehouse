...
function Shipping() {
  ...
  // untuk check kalau ada yang masih perlu di restock ga. Kalau masih perlu, send buttonnya disabled
  setTimeout(() => {
    checkCanSend();
  }, 100);
  ...
  const checkCanSend = () => {
    for (let i = 0; i < shippingData.length; i++) {
      if (
        shippingData[i].city == shippingData[i].location &&
        shippingData[i].quantity > shippingData[i].stock
      ) {
        console.log(shippingData[i].location + " is out of stock");
        setcanSend(false);
      }
    }
  };
